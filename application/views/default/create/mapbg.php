<style>
    ...
    input[type=checkbox]:checked+label>span>img, input[type=checkbox]:checked+label>img,
    input[type=radio]:checked+label>span>img, input[type=radio]:checked+label>img {
        border: 1px solid #fff;
        box-shadow: 0 0 3px 3px #090;
    }

    input[type=checkbox]+label>span>img, input[type=checkbox]+label>img,
    input[type=radio]+label>span>img, input[type=radio]+label>img {
        width: 32px;
        height: 32px;
    ...
    <div class="row">
                                    ...
                                    <?php if (!empty($listBackground)) foreach ($listBackground as $key => $item) : $key = $key + 1; ?>
                                        ...
                                        <label for="selectTile_<?php echo $key; ?>" style="padding: 0;margin: 0">
                                            <span style="display: -webkit-inline-box;background-image: url(/public/dungeon/images/map/background/<?php echo $item; ?>.png);background-repeat: no-repeat;background-size: cover;width:32px;height:32px;">
                                                <img src="/public/dungeon/images/map/inbackground/<?php echo $listinBackground[($key-1)]; ?>.png" width="32px" height="32px" alt="">
                                            </span>
                                        </label>
                ...
                <div class="foot">
                    <?php if (!empty($icon_map)) foreach ($icon_map as $key => $it) :  ?>
                        ...
                        <label for="change_<?php echo $key; ?>" style="padding: 0;margin: 0" data-toggle="popover" title="" data-content="<?php echo $it; ?>">
                            <img src="/public/dungeon/images/map/inbackground/<?php echo $it; ?>" />
                        </label>
                    <?php endforeach; ?>
                </div>
            </form>
        ...
        <div class="col-4">
            <?php foreach ($map_all as $k => $imap) : ?>
                <li><a href="<?php echo base_url('create/mapbg?Emap=' . $k) ?>"><?php echo $imap['title']; ?></a></li>
            <?php endforeach; ?>
        </div>
    </div>
</div>